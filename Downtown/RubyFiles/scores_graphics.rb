...
class Users
    ...
    def display()
        ...
        @replay_text = Text.new("Replay", x: 135, y: 400, width: 200, height: 150, color: 'black', size: 35, z: 30)
    end
    ...
    def over (x, y)
        if (@replay_button.contains? x, y) 
            ...
            @replay_button.width = 300
            @replay_button.height = 165
            @replay_text.size = 45
            # @image_border.add
            # puts "changing @convocation when hovered "
        else 
            @replay_button.width = 250
            @replay_button.height = 150
            @replay_text.size = 35
            # @image_border.remove
        end
    end 
